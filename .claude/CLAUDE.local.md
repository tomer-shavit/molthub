...
Molthub is an open-source, self-hosted control plane for running and operating a swarm of Moltbots. It provides a single place to provision Moltbot instances, configure them consistently, attach channels and integrations, enforce security guardrails, roll out changes safely, and get fleet-level visibility into health, logs, traces, cost, and quality. It integrates tightly with Moltbot's real control surfaces: the Gateway WebSocket protocol, the `moltbot.json` config model, channel auth flows, health/doctor diagnostics, and systemd/launchd service management.
...
│   ├── gateway-client/       # Moltbot Gateway WebSocket client (new)
...
- **BotInstance**: A single Moltbot with its own config, Gateway connection, profile, and deployment target
- **Template**: Starting point configs ("WhatsApp Personal Bot", "Discord Server Bot", etc.) that generate valid `moltbot.json`
...
- **Deployment Target**: Where a Moltbot runs — local (systemd/launchd), remote VM (SSH), Docker, ECS Fargate, Cloud Run, ACI, Kubernetes
- **Gateway Connection**: WebSocket link to a running Moltbot instance (port 18789) for config apply, health, status, and logs

## Moltbot Integration Essentials

- **Gateway WebSocket** on default port 18789 is the canonical transport for control and telemetry
- **Config model**: JSON5 at `~/.clawdbot/moltbot.json`, schema-validated at startup. Sections: agents, sessions, messages, channels (11 types), tools, sandbox, skills, plugins, gateway, logging, bindings
...
- **Health**: `health` method returns structured snapshot, `status` returns summary, `moltbot doctor` for diagnostics
- **Multi-instance isolation**: Use `--profile` to scope config/state/workspace. Port spacing must be 20+ apart
- **Service management**: macOS LaunchAgent (`bot.molt.<profile>`), Linux systemd user service (`moltbot-gateway-<profile>.service`)
- **Signals**: SIGTERM for graceful shutdown, SIGUSR1 for hybrid reload (config-only changes)
- **Channel auth**: WhatsApp uses QR pairing via `moltbot channels login`. WhatsApp and Telegram require Node.js (not Bun)
...
- Moltbot config changes should use `config.apply` (with baseHash for optimistic concurrency) or `config.patch`, never direct file writes
...
Transformation from cloud-generic container orchestrator to Moltbot-native operator layer is in progress. Two phases with 10 work packages:

**Phase 1 (foundation, can run in parallel):**
- WP-01: Moltbot Config Schema & Types (`packages/core`)
...
- WP-05: Policy Rules & Security (Moltbot three-layer security model)
- WP-06: Database Schema Migration (new models: GatewayConnection, MoltbotProfile, ChannelAuthSession, DeploymentTarget, SecurityAuditResult, HealthSnapshot)

**Phase 2 (depends on Phase 1 outputs):**
- WP-07: Reconciler V2 — Gateway WS config apply + deployment targets (depends: WP-01, 02, 03, 06)
- WP-08: Templates & Config Generator — valid `moltbot.json` generation (depends: WP-01, 04)
- WP-09: Web UI Moltbot Pages — Gateway status, health, config editor, channel auth (depends: WP-01, 06)
- WP-10: Health & Observability Dashboard — real Moltbot health polling + diagnostics (depends: WP-02, 06)
