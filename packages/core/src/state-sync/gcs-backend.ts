...
export class GCSStateSyncBackend implements StateSyncBackend {
  readonly type = "gcs" as const;

  constructor(
    private readonly config: GCSBackendConfig,
    private readonly client: GCSClientAdapter,
  ) {}
  ...
  async backup(options: SyncOptions): Promise<SyncResult> {
    ...
    try {
      ...
      await this.client.uploadObject({
        ...
        name: this.objectName(options.instanceId, "state.dat"),
        data: packed,
      ...
      await this.client.uploadObject({
        bucket: this.config.bucket,
        name: this.objectName(options.instanceId, "state.meta.json"),
