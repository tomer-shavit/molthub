...
function makeConfig(overrides?: Partial<CloudflareWorkersConfig>): CloudflareWorkersConfig {
  return {
    ...
    workerName: "moltbot-test-worker",
...
function makeFullConfig(): CloudflareWorkersConfig {
  return makeConfig({
    r2BucketName: "moltbot-state-bucket",
...
describe("env-mapper", () => {
  describe("mapEnvironment", () => {
    it("maps minimal config to container env, secrets, and vars", () => {
      ...
      // Container env
      expect(result.containerEnv.MOLTBOT_GATEWAY_TOKEN).toBe("test-gateway-token-abc");
      expect(result.containerEnv.MOLTBOT_GATEWAY_PORT).toBe("18789");
      expect(result.containerEnv.CLAWDBOT_CONFIG_PATH).toBe("/app/config/moltbot.json");
      expect(result.containerEnv.CLAWDBOT_STATE_DIR).toBe("/app/state");

      // Worker secrets
      expect(result.workerSecrets.MOLTBOT_GATEWAY_TOKEN).toBe("test-gateway-token-abc");

      // Worker vars
      expect(result.workerVars.WORKER_NAME).toBe("moltbot-test-worker");
    ...
    it("maps R2 config to secrets and vars", () => {
      ...
      expect(result.workerVars.R2_BUCKET_NAME).toBe("moltbot-state-bucket");
    });
  ...
  describe("isSecretKey", () => {
    it("identifies secret keys correctly", () => {
      ...
      expect(isSecretKey("MOLTBOT_GATEWAY_TOKEN")).toBe(true);
  ...
  describe("getSecretEntries", () => {
    it("returns entries for defined secrets only", () => {
      ...
      expect(entries).toContainEqual(["MOLTBOT_GATEWAY_TOKEN", "token"]);
...
describe("wrangler-generator", () => {
  describe("generateWranglerConfig", () => {
    it("generates wrangler.jsonc with correct worker name and account", () => {
      const config = makeConfig();
      const vars = { WORKER_NAME: "moltbot-test-worker", GATEWAY_PORT: "18789" };
      const output = generateWranglerConfig(config, vars);

      expect(output.wranglerJsonc).toContain("moltbot-test-worker");
    ...
    it("includes container/sandbox configuration", () => {
      ...
      expect(output.wranglerJsonc).toContain("moltbot-gateway");
    ...
    it("generates a valid Dockerfile", () => {
      ...
      expect(output.dockerfile).toContain("start-moltbot.sh");
    ...
    it("generates a valid start script", () => {
      ...
      expect(output.startScript).toContain("moltbot gateway --port 18789");
      expect(output.startScript).toContain("SIGTERM");
      expect(output.startScript).toContain("MOLTBOT_CONFIG_JSON");
    });
  });
...
describe("CloudflareWorkersTarget", () => {
  ...
  it("returns default workers.dev endpoint when no custom domain", async () => {
    ...
    expect(endpoint.host).toContain("moltbot-test-worker");
